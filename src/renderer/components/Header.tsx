...
export const Header: React.FC = () => {
  ...
  const toggleColumnSettings = useAppStore(
    (state) => state.toggleColumnSettings,
  );
  ...
  const setIncludeCompatible = useAppStore(
    (s) => (s as any).setIncludeCompatible,
  );
  ...
  useEffect(() => {
    (async () => {
      try {
        const value = await window.electronApi.settings.get(
          "search.includeCompatible",
        );
  ...
  return (
    <header className="bg-white border-b border-gray-200 px-6 py-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">
              Historique des prix - Pièces Auto
            </p>
          </div>
        </div>
      ...
      {/* Search Bar centered under Actions */}
      <div className="mt-3 mx-auto w-full max-w-lg ">
        <div className="relative w-full">
          ...
          <input
            ...
            className="w-full pl-10 pr-10 py-2 text-sm border border-gray-300 rounded-md min-w-0"
          />
